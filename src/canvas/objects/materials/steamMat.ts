...
export default function createSteamMat(opacityProgress: THREE.Node<"float">) {
  ...
  const billboardViewPos = centerView.xyz.add(vec3(offsetX, offsetY.oneMinus(), 0.0));
  ...
  const sway = sin(
    v
      .mul(Math.PI * 4.5) // 구부러지는 주기
      .add(time.mul(4.5)) // 시간에 따라 흐름
  ).mul(positionLocal.y.oneMinus().mul(0.7)); // 흔들림 강도
  const sway2 = sin(
    v
      .mul(Math.PI * 1.3)
      .add(time.mul(2.0))
  ...
  const final = warpedU.abs().oneMinus().sub(0.49).pow(3).mul(vertFade).mul(vertFade2);

  // steamMat.colorNode = color("#98978b");
  steamMat.colorNode = color("white");
  steamMat.alphaTestNode = step(final, 0.1);
  steamMat.opacityNode = opacityProgress;

  // steamMat.mrtNode = mrt({ bloomIntensity: float(1), screenIntensity: float(0) });
