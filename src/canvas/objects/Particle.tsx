...
export default function Particle() {
  ...
  const mesh = useMemo(() => {
    ...
    const seed3 = hash(instanceIndex.add(2)).toVar();
    ...
    // 속도/위상도 인스턴스별로
    const speed = seed2.mul(0.5).add(0.2);
    const phase = seed.mul(6.28);
    const amp = seed3.mul(1.5).add(0.5);
    ...
    // X는 좌우로 흔들림
    const x = baseX.add(time.mul(0.5).add(phase).sin().mul(amp));

    const centerView = cameraViewMatrix.mul(vec4(x, y, baseZ, 1.0));

    const randSize = seed3.mul(0.4).add(0.3);
    const angle = seed3.mul(Math.PI * 2);
    const localOffset = rotate(vec3(positionGeometry.xy, 0.0), vec3(0, 0, angle));
